...
func (m *MMU) Map() {
	...
	for i := 0; i < 32768; i++ {
		m.wRAM[i] = uint8(rand.Intn(256))
	}

	// set initial tile data
	// every other byte is 0x00, whilst the other byte is set from initialTileData
	for i := 0; i < len(initialTileData)*2; i += 2 {
		m.Video.Write(uint16(0x8000+i+16), initialTileData[i/2])
	}

	// set initial tile map
	for i := 0; i < len(initialMapData); i++ {
		m.Video.Write(uint16(0x9904+i), initialMapData[i])
	}
}
